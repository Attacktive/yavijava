...
public class IoFilterManager extends ManagedObject {
	 ...
	 *
	 * @param vibUrl - The URL that points to the IO Filter VIB package.
	 * @param compRes - The compute resource to install the IO Filter on. "compRes" must be a cluster.
	 ...
	 * compute resource.
	 * @throws com.vmware.vim25.AlreadyExists - Thrown if another VIB with the same name and vendor has been installed.
	 * @throws com.vmware.vim25.InvalidArgument - Thrown if "compRes" is a standalone host.
	 * @throws com.vmware.vim25.RuntimeFault - Thrown if any type of runtime fault is thrown that is not covered by the other faults; for example,
	 ...
	 * @param compRes - The compute resource. "compRes" must be a cluster.
	 ...
	 *
	 * @param filterId - ID of the filter.
	 * @param cluster - The compute resource to install the IO Filter on. "compRes" must be a cluster.
	 * @return - This method returns a Task object with which to monitor the operation. The task is set to success if
	 ...
	 *
	 * @param filterId - ID of the filter.
	 * @param host - The host to fix the issues on.
	 * @return - This method returns a Task object with which to monitor the operation. The task is set to success if
	 * all the errors related to the filter are resolved on the cluster. If the task fails, first check error to
	 * see the error. If the error indicates that issues persist on the cluster, use QueryIoFilterIssues to get
	 * the detailed errors on the hosts in the cluster. The dynamic privilege check will ensure that the
	 ...
	 * privilege check ensures that the user must have Host.Config.Patch privilege for all the hosts in the
	 * compute resource.
	 * @throws com.vmware.vim25.FilterInUse Thrown if the filter to be uninstalled is being used by a virtual disk.
	 * @throws com.vmware.vim25.InvalidState Thrown if "compRes" is a cluster and DRS is disabled on the cluster.
	 * @throws com.vmware.vim25.NotFound Thrown if the filter is not installed on the cluster. - Thrown if another VIB with the same name and
